...
public class CutLinkTable {
    ...
    public KeyValueRow lookup(KeyValue key) {
        return getReference(key);
    }

    public KeyValueRow lookup(Column prm, List<String> values) {
        ...
        KeyValueRow ret = new KeyValueRow(m_table.getColumns(prm));
    ...
    private void addReference(KeyValue prm, ArrayList<KeyValue> row) {
        KeyValueRow kvRow = getReference(prm);
        if (kvRow.add(row) > 0)
            setReference(prm, kvRow);
    }

    private void setReference(KeyValue prm, KeyValueRow row) {
    ...
    private KeyValueRow getReference(KeyValue prm) {
    ...
    private KeyValueRow getRowFromRecord(Record record, ArrayList<Column> columns) {
        KeyValueRow ret = new KeyValueRow(columns);
        if (record != null) {
